...
const STORAGE_HABITOS = 'fuvi_habitos_v2';
...
export class HabitosService {
  ...
  private cargarHabitos(): Habito[] {
    ...
    try {
      ...
      return this.sincronizarPictogramasLavadora(habitosParseados);
    } catch {
      return this.getHabitosIniciales();
    }
  }

  private sincronizarPictogramasLavadora(habitos: Habito[]): Habito[] {
    return habitos.map((habito) => {
      const esHabitoLavadora =
        habito.id === 2 || habito.nombre.toLocaleLowerCase().trim() === 'poner lavadora';
